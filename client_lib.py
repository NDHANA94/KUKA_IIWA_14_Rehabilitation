...
class kuka_iiwa_ros_client:
    ...
    def __init__(self): # Makes kuka_iiwa ROS node
        ...
        print (Fore.GREEN  + ' Client Version: ' + version)
        print ('==========================================\n')
        ...
        rospy.Subscriber("hasError", String, self.hasError_callback)
    ...
    def hasError_callback(self, data):
        d = str(data.data).split()
        if d[0] == 'True': stat = True
        if d[0] == 'False': stat = False
        self.hasError = (stat, float(d[1]))
