...
class iiwa_socket:
    ...
    def __init__(self, ip, port):
        ...
        try:
            ...
            print (Fore.RED  , 'Error: ') + "Unable to start connection thread"
    #   ~M: __init__ ==========================
    ...
    def socket(self, ip, port):
        ...
        # os.system('clear')
        print (Fore.GREEN  + '\n==========================================')
        print (Fore.GREEN  + '<   <  < << SHEFFIELD ROBOTICS >> >  >   >')
        print (Fore.GREEN  + '==========================================')
        print (Fore.GREEN  + ' KUKA API for ROS')
        print (Fore.GREEN  + ' Server Version: ' + version)
        print (Fore.GREEN  +' ==========================================\n')


        print (Fore.CYAN  + 'Starting up on:', 'IP:', ip, 'Port:', port)
        try:
            sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            sock.bind(server_address)
        except:
            print (Fore.RED + 'Error: ' + "Connection for KUKA cannot assign requested address:", ip, port)
            os._exit(-1)
        ...
        print (Fore.CYAN  + 'Connection from', client_address)
        self.isconnected = True
        last_read_time = time.time()
        print("Kuka iiwa is connected: ", self.isconnected)

        while self.isconnected:
            try:
                data = self.connection.recv(self.BUFFER_SIZE)
                last_read_time = time.time()    # Keep received time
                data_ = str(data, 'utf-8') # convert data type from byte to string
                
                # Process the received data package
                for pack in data_.split('>'):  # parsing data pack
                    cmd_splt = pack.split()
                ...
                if elapsed_time > 5.0:  # Didn't receive a pack in 5s
                    ...
                    self.isconnected = False
                    print (Fore.LIGHTRED_EX  + 'No packet received from iiwa for 5s!')

        self.connection.shutdown(socket.SHUT_RDWR)
        self.connection.close()
        sock.close()
        self.isconnected = False
        print (Fore.LIGHTRED_EX  +'Connection is closed!')
    #   ~M: Connection socket ===================
...
class kuka_iiwa_ros_node:
    ...
    def __init__(self, ip, port): # Makes kuka_iiwa ROS node
        ...
        # debug = rospy.Publisher('mob_app_debug_msg', String, queue_size=10)
...
def read_conf():
    ...
    if os.path.isfile(f_conf):
        ...
        if len(IP.split('.'))!=4 or Port<=0:
            print (Fore.RED  +'Error:'), "conf.txt doesn't include correct IP/Port! e.g. server 172.31.1.50 port 1234"
            exit()
    else:
        print (Fore.RED  +'Error:'), "conf.txt doesn't exist!"
        exit()
